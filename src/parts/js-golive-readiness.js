...
async function _goLiveEdgeFunctionsItem(cfgUrl){
  ...
  const names = ['create-user','admin-ops','send-notification','widget-data','calendar-feed'];
  const results = [];
...
async function renderGoLiveChecklist(){
  ...
  const items = [
    ...
    _goLiveStatus(!_isAdminOrSU() || _adminMfaOk() ? 'ok' : 'fail', 'Admin MFA enforcement', !_isAdminOrSU() ? 'Current session is not an admin session.' : (_adminMfaOk() ? `Admin session is ${_MFA_STATUS?.currentLevel || 'aal2'}.` : 'Admin tools are blocked until this session reaches aal2.')),
