  ...
  document.getElementById('drc-body').innerHTML=cells.map(c=>{
    ...
    _shiftsOnDate('dr', c.date).filter(_matchDRShift).forEach(s=>{
      ...
      if(s.shift==='Home'||s.autoHome){
        ...
        // Escape every admin-editable string interpolated into a title
        // attribute — without this a notes value containing " would
        // break out of the attribute and execute injected HTML.
        evs+=`<div class="ev ${cls}${nonAnc}" title="${pnameHtml(s.physId)} — ${escHtml(s.shift)} Shift (Home)${escHtml(wrvuTip)}${s.notes?' · '+escHtml(s.notes):''}">
          <div style="font-weight:700;font-size:11px">🌙 ${pshort(s.physId)}</div>
          <div style="font-size:9px;opacity:.85">${escHtml(s.shift)} ${badge}${wrvuG > 0 ? ` · ${wrvuG}` : ''}</div>
        </div>`;
      } else {
        const wrvuG = (typeof getShiftWRVUGoal === 'function') ? getShiftWRVUGoal(s, s.physId, 'dr') : 0;
        const wrvuTip = wrvuG > 0 ? ` · wRVU goal ${wrvuG}` : '';
        evs+=`<div class="ev ${cls}${nonAnc}" title="${pnameHtml(s.physId)} — ${escHtml(s.shift)} Shift @ ${escHtml(s.site||'')}${s.slotLabel?' ['+escHtml(s.slotLabel)+']':''}${s.sub?' ('+escHtml(s.sub)+')':''}${escHtml(wrvuTip)}">
          <div style="font-weight:700;font-size:11px">${pshort(s.physId)}</div>
          <div style="font-size:9px;opacity:.85">${escHtml(s.shift)} Shift${wrvuG > 0 ? ` · ${wrvuG} wRVU` : ''}</div>
          <div style="font-size:9px;opacity:.7">${escHtml(siteSh)}${escHtml(slotLbl)}${subLbl?' · '+escHtml(subLbl):''}${_driveTimeBadge(s.physId,s.site,s.shift,s.date,'DR')}</div>
        </div>`;
      }
    });
    if(!fsh||fsh==='Weekend') _shiftsOnDate('wkn', c.date).filter(w => (!fpid||w.physId===fpid) && (!fsite||w.site===fsite) && !fsub).forEach(w=>{
      evs+=`<div class="ev ewk" title="${pnameHtml(w.physId)} — Weekend Call @ ${escHtml(w.site||'')}">
    ...
    if(!fsh||fsh==='Holiday') S.holidays.filter(h => h.date===c.date && h.group==='DR' && (!fpid||h.physId===fpid) && !fsite && !fsub).forEach(h=>{
      evs+=`<div class="ev ehol" title="${pnameHtml(h.physId)} — Holiday Call: ${escHtml(h.name||'')}">
