...
function renderMySched(){
  ...
  // rs-v100: "next 4 weeks workload" mini-widget. _renderMyWorkload was
  // fully implemented (its comment even says "called from renderMySched")
  // but neither the container nor the call ever existed — found by the
  // zero-reference sweep. Container only renders for linked physicians;
  // the renderer no-ops without CU.physId.
  if(CU?.physId) html += `<div class="card" id="my-workload" style="margin-top:12px"></div>`;
  document.getElementById('my-content').innerHTML=html;
  if(CU?.physId && typeof _renderMyWorkload === 'function'){
    try{ _renderMyWorkload(); }catch(e){ if(typeof _logError === 'function') _logError('_renderMyWorkload', e?.message||String(e), e); }
  }
}
