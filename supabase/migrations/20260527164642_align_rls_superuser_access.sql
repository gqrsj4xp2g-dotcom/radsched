-- Align live RLS with the app's role model: app_metadata.role=superuser
-- has all admin privileges, while normal users are scoped to their
-- app_metadata.practiceId. This also removes the legacy "missing
-- practiceId can access main" fallback.
...
CREATE POLICY practices_insert_privileged ON public.practices
  ...
  WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') IN ('admin','superuser'));

CREATE POLICY practices_update_privileged ON public.practices
  FOR UPDATE TO authenticated
  USING ((auth.jwt() -> 'app_metadata' ->> 'role') IN ('admin','superuser'))
  WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') IN ('admin','superuser'));
